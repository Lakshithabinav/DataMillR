    ...
    const timer = setTimeout(() => {
      if (companyName.trim() !== '') {
        axios
          ...
          .get(`${config.BASE_URL}/admin/search-company-name?companyName=${encodeURIComponent(companyName)}`)
          .then((res) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(
        ...
        `${config.BASE_URL}/admin/create-table`,
  ...
  const handleMapping = async (e) => {
    ...
    try {
      const res = await axios.post(
        ...
        `${config.BASE_URL}/admin/register-device`,
        mappingPayload
      );
