...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (companyName.trim() !== '') {
        axios.get(`${config.BASE_URL}/admin/search-company-name?companyName=${encodeURIComponent(companyName)}`)
          .then((res) => {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${config.BASE_URL}/admin/veyor-table`,
    ...
    const packinghandleRegister = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await axios.post(
        `${config.BASE_URL}/admin/packing-table`,
        qs.stringify(payload),
        {
          headers: { "Content-Type": "application/x-www-form-urlencoded" },
        }
  ...
  const handleMapping = async () => {
    ...
    try {
      const res = await axios.post(`${config.BASE_URL}/admin/register-device`, payload);
      const data = res.data;
    ...
    <div className="dashboard-form-container">
      ...
      <form onSubmit={handleRegister}>
        ...
        <div className="form-section-margin">
          <button type="submit">Flowvayor Register</button>
        </div>

      <div id="packingreg" className="form-section-margin">
        {/* <button type="submit">Register</button> */}
        <button type="submit" onClick={packinghandleRegister}>Packing Register</button>
      </div>
        ...
        <div className="button-row">
          <button type="button" onClick={handleMapping}>Mapping</button>
